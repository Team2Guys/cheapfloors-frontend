      ...
      <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-[310px] sm:w-[350px] xl:w-[400px] 2xl:w-[500px] bg-white border border-[#afa183] border-opacity-30 rounded-2xl mt-2 sm:mt-6 z-20">
        <div className="flex justify-end mb-2 sticky top-0 p-2 z-30 bg-white rounded-t-2xl">
          <svg
            ...
            onClick={() =>
              expandable ? closeSearch() : setIsProductListOpen(false)
            }
            width="28"
            height="28"
            viewBox="0 0 48 48"
  ...
  const formContent = (
    <>
      <input
        ...
        onFocus={() => expandable && setIsExpanded(true)}
        onChange={(e) => {
      ...
      {expandable && isExpanded && (
        <button
          ...
          onClick={closeSearch}
          className="absolute right-3 top-1/2 -translate-y-1/2 text-2xl leading-none text-gray-600 hover:text-black"
        >
          &times;
        </button>
      )}
      {isProductListOpen && productListMarkup}
    </>
  );
    ...
    return (
      ...
      >
        <form
          ...
          className={`absolute right-0 top-1/2 -translate-y-1/2 font-inter transition-[width] duration-300 ease-in-out ${isExpanded ? 'w-[22rem] 2xl:w-[28rem] z-40' : 'w-full'
            }`}
        >
          {formContent}
        </form>
